...
- 2026-08-16 · `PRODUCCIÓN` · Hay DOS implementaciones del layout y NO son intercambiables. El template documentado es HTML/CSS con auto-fit por JavaScript: necesita navegador, y el contenedor no tiene ninguno, así que solo corre en Apify. El render local es otra cosa (dibujar con Pillow). Local para iterar el encuadre (instantáneo vs 2-4 min); HTML por Apify para la tapa final, que es el template medido contra la referencia.
- 2026-08-16 · `PRODUCCIÓN` · ⚠️ SIN RESOLVER: no existe una implementación local documentada con las constantes exactas. Mientras no exista, una tapa renderizada local hay que verificarla contra assets/REFERENCE-carousel.png antes de entregar. Si difiere del molde, rehacerla por el camino HTML.
