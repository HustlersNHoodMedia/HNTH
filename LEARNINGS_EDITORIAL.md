...
- 2026-08-14 · `PRODUCCIÓN` · El QC numérico NO alcanza. Un post pasó todas las medidas (handle 192, titular en 639, margen 54, sin desborde) y salió con el título corrido y el degradado mal. Hay dos tapas de referencia en assets/ del repo (REFERENCE-split.png y REFERENCE-single.png): bajarlas y comparar lado a lado con view antes de entregar. Si no se parece a la referencia, no se entrega.
...
- 2026-08-14 · `PRODUCCIÓN` · 🚨 CAUSA RAÍZ DEL DEGRADADO ALTO: la referencia de 2Pac mide 1080x1260 y el canvas de producción es 1080x1350. Toda coordenada VERTICAL medida sobre la referencia hay que multiplicarla por 1,0714. Copiar valores de Y en píxeles absolutos deja todo demasiado arriba. Las coordenadas horizontales no se escalan (ambos son 1080 de ancho).
- 2026-08-14 · `PRODUCCIÓN` · Valores corregidos del degradado: inicio y=555, negro sólido y=690 (en la referencia son 518 y 644). El manual decía 330→560, o sea 130px más arriba de lo que corresponde: eso se comía la mitad de la foto. Se corrigió a ojo tres veces sin converger porque el número de origen estaba mal escalado.
- 2026-08-14 · `PRODUCCIÓN` · DECISIÓN: el canvas vuelve a 1080x1260, el tamaño que la marca usó siempre. Referencia y salida ahora miden lo mismo, así que las coordenadas se usan tal cual y desaparece toda conversión. El canvas de 1350 fue el origen del error de escala.
- 2026-08-14 · `PRODUCCIÓN` · UNA SOLA referencia: assets/REFERENCE-carousel.png (la tapa de 2Pac, hecha a mano por el operador). Se borraron las otras dos porque estaban en 1350 y traían el degradado mal: comparar contra ellas enseñaba el error.
- 2026-08-14 · `PRODUCCIÓN` · Split screen y círculo NO necesitan referencia propia: son una división geométrica de la misma zona de imagen. El degradado, handle, logo, titular y chip no se mueven entre variantes.
- 2026-08-14 · `PRODUCCIÓN` · El chip SWIPE LEFT es un asset: assets/SWIPE-LEFT.png (223x43, rojo sólido en los cuatro bordes). NUNCA recortarlo de un post publicado — arrastra fondo y queda con borde negro.
